"""SQLite task store for Hermes ↔ OpenCode delegations."""
...
def _default_db_path() -> str:
    return os.path.join(os.environ.get("HERMES_HOME", "/data"), "opencode_tasks.db")
...
CREATE INDEX IF NOT EXISTS idx_opencode_tasks_status ON opencode_tasks(status);
CREATE INDEX IF NOT EXISTS idx_opencode_tasks_session ON opencode_tasks(opencode_session_id);
"""
...
class TaskStore:
    ...
    def update_task(self, task_id: str, **fields: Any) -> dict[str, Any] | None:
        ...
        with self._db() as conn:
            conn.execute(f"UPDATE opencode_tasks SET {columns} WHERE id = ?", values)
        return self.get_task(task_id)

    def get_task(self, task_id: str) -> dict[str, Any] | None:
        with self._db() as conn:
            row = conn.execute("SELECT * FROM opencode_tasks WHERE id = ?", (task_id,)).fetchone()
        return self._row_to_dict(row) if row else None

    def get_task_by_session_id(self, opencode_session_id: str) -> dict[str, Any] | None:
        with self._db() as conn:
            row = conn.execute(
                "SELECT * FROM opencode_tasks WHERE opencode_session_id = ? ORDER BY updated_at DESC LIMIT 1",
                (opencode_session_id,),
            ).fetchone()
        return self._row_to_dict(row) if row else None

    def list_tasks(self, *, status: str | None = None, limit: int = 20) -> list[dict[str, Any]]:
        query = "SELECT * FROM opencode_tasks"
        params: list[Any] = []
        if status:
            query += " WHERE status = ?"
    ...
    def count_active(self) -> int:
        with self._db() as conn:
            row = conn.execute(
                "SELECT COUNT(*) AS c FROM opencode_tasks WHERE status IN ('pending', 'running')"
            ).fetchone()
